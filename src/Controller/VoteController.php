...
class VoteController extends AbstractController
{
    ...
    #[Route('/vote/submit/{surveyId}/surveypage', name: 'vote_submitpage', methods: ['POST'])]
    public function submitPage(int $surveyId, Request $request, SurveyRepository $surveyRepository, VoteRepository $voteRepository, EntityManagerInterface $entityManager): Response
    {
        $user = $this->getUser();
        ...
        return $this->redirectToRoute('survey_index', [
            'slug' => $survey->getBuilding()->getSlug(),
            '_fragment' => 'sondage' . $survey->getId()
        ]);
